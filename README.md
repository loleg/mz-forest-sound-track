...
- Starting work on a training and classification script. Currently the code closely follows [GianlucaPaolocci/Sound-classification-on-Raspberry-Pi-with-Tensorflow](https://github.com/GianlucaPaolocci/Sound-classification-on-Raspberry-Pi-with-Tensorflow) We run the training neural network on our local pc by using the training dataset as input to our 3-layer neural network. Once our network is trained, all we need is the network topology and the final set of weights saved on the Raspberry-Pi which is installed at detection point in the forest. The classification netwrok is run on the Raspberry Pi, once the microphone detects any sound and uses that sound as an input and makes predictions. 
